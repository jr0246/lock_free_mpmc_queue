...
namespace jr {
    ...
    struct entry {
        ...
        alignas(cache_line_size) rl::atomic<size_t> seq{0};
        alignas(std::max_align_t) rl::var<char> buf[sizeof(T)]{};
        ...
        constexpr void set_seq(const index_type s) noexcept {
            seq($).store(s, rl::memory_order_release);
        }

        [[nodiscard]] constexpr index_type get_seq() const noexcept { return seq($).load(rl::memory_order_acquire); }
    ...
    class lock_free_mpmc_queue : public rl::test_suite<lock_free_mpmc_queue<Data, N>, 10> {
    public:
        ...
        constexpr lock_free_mpmc_queue() : _capacity{N},
            _mask{_capacity($) - 1}, _ring{static_cast<entry<Data>*>(malloc(N * sizeof(entry<Data>)))},
    ...
    private:
        rl::var<size_type> _capacity;
        rl::var<size_type> _mask;
        rl::var<entry<Data>*> _ring;

        alignas(cache_line_size) rl::atomic<size_type> _write_index;
        alignas(cache_line_size) rl::atomic<size_type> _read_index;
